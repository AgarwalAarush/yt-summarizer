...
def create_pdf(video_url) :
    ...
    pdf.multi_cell(0, 20, get_video_title(video_url=video_url), align='C', new_x=XPos.LMARGIN)
    ...
    for i in range(num_paragraphs):
        ...
        # Shift Image Up/Down (From Y-Pos)
        # !!!!!!! Difference !!!!!!!
        if iteration == 0:
            pdf.image("resources/photos/%s/%s.jpeg" % (alphanumeric(get_video_id(video_url)), chr(image_idx + 97)), 110, y_pos + (max_height - final_img_height) / 2, width)
        else:
            pdf.image("resources/photos/%s/%s.jpeg" % (alphanumeric(get_video_id(video_url)), chr(image_idx + 97)), 10, y_pos + (max_height - final_img_height) / 2, width)
...
#         pdf.add_page()
#         y_pos = pdf.get_y()
#         pdf.multi_cell(100, 0, new_x=XPos.RIGHT)
#     # Create Text Cells & Formatting
#     y_initial = pdf.y
#     pdf.multi_cell(85, 8, current_text, border=0, new_x=XPos.LMARGIN)
#     y_final = pdf.y
